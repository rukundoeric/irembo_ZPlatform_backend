...
class AuthController {
  ...
  static async sendOneTimePassword(req, res) {
    ...
    const one_time_password = await generateToken({ password }, { type: 'one-time-password' }, '5m');
    const result = await Auth.update({ one_time_password }, { where: { user_id } });
    if (!result) return res.sendStatus(500);
    sendOneTimePassword({ email, password: one_time_password });
    res.json({
      user: { user_id, email },
  ...
  static async secondFactorAuth(req, res) {
    ...
    decodeJWT(encodedPassword, async (err, decoded) => {
      if (err) return res.status(403).json({ error: { message: 'Password is expired!' } });
